...
调用内容会发送给配置的服务商。MCP/SDK 的单个上游请求超时 30 秒，不自动重试；取消或超时不代表远端未执行或未计费。服务不持久化输入，stdout 仅承载 MCP 协议，错误不回显上游正文。供应商差异由 MCP 适配层处理；配置与适配边界见 [供应商接入](docs/providers.md)，调用方按统一工具契约及错误提示处理。
